...
export const NationalIDUtils = {
	parseEstonianIdCode: (idCode: string) => {
		...
		const genderDigitMetaInfo = getMetaInfoFromGenderDigit(
			Number(idCodeComponents.genderDigit),
		);
		...
		return {
			...
			meta: {
				...genderDigitMetaInfo,
				fullBirthYear:
					genderDigitMetaInfo.birthYearStart +
					Number(idCodeComponents.birthYearLastTwoDigits),
			},
		};
	},
	parseFinnishIdCode: (idCode: string) => {
		...
		const metaInfoFromCenturyCharacter = getMetaInfoFromCenturyCharacter(
			idCodeComponents.centuryCharacter,
		);
		...
		return {
			...
			meta: {
				...
				fullBirthYear:
					metaInfoFromCenturyCharacter.birthYearStart +
					Number(idCodeComponents.birthYearLastTwoDigits),
			},
		};
	},
	parseLatvianIdCode: (idCode: string) => {
