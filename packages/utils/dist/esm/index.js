...
const parseLatvianIdCode = (idCode) => {
    throw new Error("Latvian id code not implemented yet");
};
const parseFinnishIdCode = (idCode) => {
    const finnishIdCodeRegExp = /^(?<birthDay>[0-9]{2})(?<birthMonth>[0-9]{2})(?<birthYear>[0-9]{2})(?<centuryCharacter>[-+ABCDEFYXWVU]{1})(?<sequenceDigits>[0-9]{3})(?<checksumCharacter>[0-9ABCDEFHJKLMNPRSTUVWXY]{1})$/;
    if (!finnishIdCodeRegExp.test(idCode)) {
        throw new Error("Invalid finnish id code");
    }
    const getMetaInfoFromCenturyCharacter = (centuryCharacter) => {
        if (centuryCharacter === '+') {
            return {
                birthYearStart: 1800,
                birthYearEnd: 1899,
    ...
    return {
        ...
        meta: {
            ...
            fullBirthYear: metaInfoFromCenturyCharacter.birthYearStart + Number(idCodeComponents.birthYearLastTwoDigits),
        },
    };
};
export var NationalId;
(function (NationalId) {
    NationalId["Est"] = "estid";
    NationalId["Fin"] = "finid";
