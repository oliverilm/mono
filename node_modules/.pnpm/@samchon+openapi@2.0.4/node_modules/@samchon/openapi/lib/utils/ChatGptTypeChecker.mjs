...
(function(ChatGptTypeChecker) {
    ...
    ChatGptTypeChecker.visit = props => {
        ...
        const next = (schema, accessor) => {
            ...
            } else if (ChatGptTypeChecker.isAnyOf(schema)) schema.anyOf.forEach(((s, i) => next(s, `${accessor}.anyOf[${i}]`))); else if (ChatGptTypeChecker.isObject(schema)) for (const [key, value] of Object.entries(schema.properties)) next(value, `${accessor}.properties[${JSON.stringify(key)}]`); else if (ChatGptTypeChecker.isArray(schema)) next(schema.items, `${accessor}.items`);
        };
    ...
    const coverObject = p => Object.entries(p.y.properties ?? {}).every((([key, b]) => {
