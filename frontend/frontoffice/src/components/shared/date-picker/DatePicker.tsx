...
export function DatePicker({
    ...
    const form = useForm<DatePickerFormValues>({
        ...
        validate: {
            month: (value) => (allMonthNamesInAYear.includes(value) ? null : 'Invalid month'),
            day: (value) => (value >= 1 && value <= 31 ? null : 'Invalid day'),
            year: (value) => (value < new Date().getFullYear() ? null : 'Invalid year'),
    ...
    const tempDate = useMemo((): Date => {
        ...
        return new Date(year, allMonthNamesInAYear.indexOf(month), day, hour, minute);
    }, [form.values])
    ...
    console.log({ dayArray, daySelected: form.values.day, getDaysInMonth: getDaysInMonth(), monthSelected: form.values.month})
    return (    
        <Flex>
            <Select data={allMonthNamesInAYear} {...form.getInputProps("month")}/>
            <Select data={dayArray}  {...form.getInputProps("day")}/>
            <Select data={[currentYear, currentYear + 1].map(String)} {...form.getInputProps("year")}/>
        </Flex>
    );
}