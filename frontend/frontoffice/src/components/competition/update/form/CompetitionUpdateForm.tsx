...
export function CompetitionUpdateForm({competition, onSubmitSuccess}: Props) {
    ...
    const form = useForm<Omit<UpdateCompetition, "startingAt"> & { startingAt: Date }>({
        initialValues: {
            id: competition.id,
    ...
    const onSubmit = async (values: typeof form.values) => {
        const response = await CompetitionAPI.updateCompetition({...values, startingAt: new Date(values.startingAt).toISOString()})
    ...
    return (
        <form onSubmit={form.onSubmit(onSubmit)}>
            <Flex direction={"column"} gap={"sm"}>
                ...
                <TextInput type="date" label="startingAt" {...form.getInputProps("startingAt")} />
